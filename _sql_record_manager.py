...
class UpsertionRecord(Base):  # type: ignore[valid-type,misc]
    ...
    __tablename__ = f"dev_ai_agent_record_manager"
    ...
    namespace = Column(String(255), index=True, nullable=False)
        ...
        *,
        ...
        with self._make_session() as session:
            if self.dialect == "sqlite":
                ...
                # Note: uses mysql insert to make on_conflict_do_update work.
                # This code needs to be generalized a bit to work with more dialects.
                mysql_insert_stmt: MysqlInsertType = mysql_insert(
                    UpsertionRecord
                ).values(records_to_upsert)
                stmt = mysql_insert_stmt.on_duplicate_key_update(  # type: ignore[assignment]
